...
from thermo_calc import thermo_calc_1
import numpy as np
import time

def model1(phi, pressure, max_it=50):
    """
    ...
    a        = np.array([[2,1],
                    ...
                    [1,1]])
    b        = [2, 2*a_coef]
    y        = np.array(initialGuess(1, a_coef))
    #***********************************************************************************************
    ...
    for k in range(1, max_it):
        ...
        hs_dict, hf_dict, g_f_dict = thermo_calc_1(enthalpy_298_15_dict,
                                                   enthalpy_dict,
                                                   entropy_dict,
                                                   species_str_list,
                                                   T)
        ...
        deltaH_R1 = np.dot(sol, [hs_dict[s] for s in species_str_list]) + sol[0] * species_obj_dict["H2O"].hf_298_15 + sol[3] * species_obj_dict["OH"].hf_298_15
        ...
        deltaH_Rad = np.dot(sol, [hs_dict[s] for s in species_str_list]) + sol[0] * species_obj_dict["H2O"].hf_298_15 + sol[3] * species_obj_dict["OH"].hf_298_15
    ...
    return [Tad, mol_fraction, deltaH_Rad, iterations]

if __name__ == '__main__':
    start = time.time()
    results = model1(1.0, 10)
    end = time.time()
