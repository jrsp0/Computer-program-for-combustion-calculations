###############################################################################################
# 
# -> Este script faz o cálculo da temperatura adiabática chama para o modelo 1 para uma dada razão de
#    equivalência e pressão (em bar)
#
###############################################################################################

from iterateLagrange import iterateLagrange
# from initial_guess import initialGuess2
from initialGuess import initialGuess_CH4_O2
from Species_coefficients import Species_coefficients
from thermo_calc import thermo_calc_4
import numpy as np
import time

def exec_calc_CH4(phi, pressure, max_it=50):
    """
    Função que faz o cálculo da temperatura adiabática de chama
    (e consequentemente da composição de equilibrio química)

    """

    R = 8.3145

    species_str_list = ["H2O", "CO2", "O2", "CO", "O", "H2", "OH", "H"]
    Species_coefficients.get_txt_file_content()
    species_obj_dict = {s: Species_coefficients(s) for s in species_str_list}

    # CH4
    CH4_dict = {"CH4" : Species_coefficients("CH4")}
    CH4_dict["CH4"].get_coeff()


    for values in species_obj_dict.values():
        values.get_coeff()
    enthalpy_298_15_dict = {s: species_obj_dict[s].enthalpy(298.15) for s in species_str_list}

    # m_ox / m_fuel
    mass_ratio = 4 / phi
    
    a_coef = mass_ratio / 2

    #***********************************************************************************************
    # Parâmetros do modelo 1

    i        = 8
    j        = 3
    a        = np.array([[0,2,1],
                         [1,0,2],
                         [0,0,2],
                         [1,0,1],
                         [0,0,1],
                         [0,2,0],
                         [0,1,1],
                         [0,1,0]])
    b        = [1, 4, 2 * a_coef]
    y        = np.array(initialGuess_CH4_O2(phi, a_coef))
#     y = np.array([1.53552428, 0.46471903, 0.28694797, 0.53528036, 0.11298964, 0.23910202,
#  0.3065884,  0.14415641])
    #***********************************************************************************************

    # T1 < Tad    
    T1 = 2500
    # T2 > Tad
    T2 = 4000

    for k in range(1, max_it):
        
        iterations = k

        # Para o Método da Bisseção:
        Tad = (T1 + T2) / 2

        # ============================== chemical equilibrium for T1 ==============================

        T = T1

        enthalpy_dict = {s: species_obj_dict[s].enthalpy(T) for s in species_str_list}
        entropy_dict = {s: species_obj_dict[s].entropy(T) for s in species_str_list}

        hs_dict, hf_dict, g_f_dict = thermo_calc_4(enthalpy_298_15_dict,
                                                   enthalpy_dict,
                                                   entropy_dict,
                                                   species_str_list,
                                                   T)

        g_RT = [1/R * g_f_dict[s] + np.array(enthalpy_298_15_dict[s] / (R * T)) for s in species_str_list]

        sol = iterateLagrange(pressure,g_RT,i,j,a,b,y,max_it)

        deltaH_R1 = np.dot(sol, [hs_dict[s] for s in species_str_list]) + \
                sol[0] * species_obj_dict["H2O"].hf_298_15 + \
                sol[1] * species_obj_dict["CO2"].hf_298_15 + \
                sol[3] * species_obj_dict["CO"].hf_298_15 + \
                sol[4] * species_obj_dict["O"].hf_298_15 + \
                sol[6] * species_obj_dict["OH"].hf_298_15 + \
                sol[7] * species_obj_dict["H"].hf_298_15 - \
                CH4_dict["CH4"].hf_298_15

        # ============================== chemical equilibrium Tad ==============================

        T = Tad

        enthalpy_dict = {s: species_obj_dict[s].enthalpy(T) for s in species_str_list}
        entropy_dict = {s: species_obj_dict[s].entropy(T) for s in species_str_list}

        hs_dict, hf_dict, g_f_dict = thermo_calc_4(enthalpy_298_15_dict,
                                                   enthalpy_dict,
                                                   entropy_dict,
                                                   species_str_list,
                                                   T)

        g_RT = [1/R * g_f_dict[s] + np.array(enthalpy_298_15_dict[s] / (R * T)) for s in species_str_list]

        sol = iterateLagrange(pressure,g_RT,i,j,a,b,y,max_it)

        deltaH_Rad = np.dot(sol, [hs_dict[s] for s in species_str_list]) + \
                sol[0] * species_obj_dict["H2O"].hf_298_15 + \
                sol[1] * species_obj_dict["CO2"].hf_298_15 + \
                sol[3] * species_obj_dict["CO"].hf_298_15 + \
                sol[4] * species_obj_dict["O"].hf_298_15 + \
                sol[6] * species_obj_dict["OH"].hf_298_15 + \
                sol[7] * species_obj_dict["H"].hf_298_15 - \
                CH4_dict["CH4"].hf_298_15

        # print(f'deltaH_R1: {deltaH_R1}')
        # print(f'deltaH_Rad: {deltaH_Rad}')

        # Método da Bisseção
        if deltaH_R1 * deltaH_Rad < 0:
            T2 = Tad
        else:
            T1 = Tad

        # Verificação do erro
        if 0 < abs(deltaH_Rad) < 100:
            break
        else:
            continue

    mol_fraction = sol/sum(sol)

    return [Tad, mol_fraction, deltaH_Rad, iterations, sol]

if __name__ == '__main__':
    start = time.time()
    results = exec_calc_CH4(1.14, 10)
    end = time.time()
    print(f"Adiabatic flame temperature {results[0]}")
    print(f"Chemical equilibrium composition {results[1]}")
    print("Mol Number:")
    print(results[-1])
    print(end - start)